...
x_buf = x_buf .^ 2; % x(n + mh)^2
x_win = x_buf .* W; % x(n + mh)^2 * w(n)
...
% Deriving the basic envelope function
Em = 1/win_size * blank;
m = 1:length(Em);

% Here, the log energy derivative is derived by taking the first order
% derivative of the log of the derived envelope function. 

% Delta values
delta_Em = diff(log(Em));
delta_m = diff(m);

% Differentiation
n_t_le = (delta_Em)./(delta_m);
